...
classdef ClickHouseClient < handle
    ...
    methods
        function obj = ClickHouseClient(host, port, username, password, options)
            ...
            if ~isa(options.compression, 'Compression') || ~isscalar(options.compression)
                error("ClickHouse:badOption", ...
                    "options.compression must be a scalar Compression enum value " + ...
                    "(Compression.None, Compression.LZ4, or Compression.ZSTD).");
            end
            ...
            obj.ptr = clickhouse_mex('connect', ...
        ...
        function result = ping(obj)
            ...
            result = clickhouse_mex('ping', obj.ptr);
        end
        ...
        function result = query(obj, sql)
            ...
            try
                s = clickhouse_mex('query', obj.ptr, char(sql));
            catch ME
                if obj.maxRetries <= 0
                    rethrow(ME);
                end
